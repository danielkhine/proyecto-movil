...
function SettingsItem({ title, onPress, collapsibleActive, weight=false, weightNumber, fontSize=18, fontWeight= 500 }) {
  ...
  return (
    <TouchableOpacity style={styles.container} onPress={onPress}>
      <View style={styles.item}><Text style={[{fontSize: fontSize, fontWeight: fontWeight}]}  > {weight ? <Text style={{ color: '#FFD700', fontWeight: 700 }}>{weightNumber}</Text> : null}{ weight ? "x " + title : title }</Text></View>
      {noCollapsible ? (
